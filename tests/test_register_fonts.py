...
def test_registers_new_font(
        monkeypatch: pytest.MonkeyPatch, valid_font: Path
) -> None:
    """When font is not registered yet, it should be registered."""
    called = {"count": 0}

    def fake_get_font(name: str) -> NoReturn:
        raise KeyError  # not registered yet
    ...
    spec = FontSpec(name="DummyX", file_path=valid_font)
